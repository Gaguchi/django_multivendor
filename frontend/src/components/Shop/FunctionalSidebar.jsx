...
export default function FunctionalSidebar({ 
  ...
  const [localFilters, setLocalFilters] = useState({
    categories: [],
    brands: [], // Add brands to local filters
    priceRange: [priceRange.min, priceRange.max], // Change to priceRange array
    ...currentFilters
  })

  // Available filter options - Remove color and size options

  // Initialize filters from URL params
  useEffect(() => {
    const urlFilters = {
      categories: searchParams.get('categories')?.split(',').filter(Boolean) || [],
      brands: searchParams.get('brands')?.split(',').filter(Boolean) || [], // Get brands from URL params
      priceRange: [
        parseInt(searchParams.get('price_min')) || priceRange.min,
        parseInt(searchParams.get('price_max')) || priceRange.max
      ],
    }
  ...
  const updateFilters = (newFilters) => {
    ...
    if (newFilters.brands.length) params.set('brands', newFilters.brands.join(',')) // Update brands in URL params
    if (newFilters.priceRange[0] > priceRange.min) params.set('price_min', newFilters.priceRange[0])
    if (newFilters.priceRange[1] < priceRange.max) params.set('price_max', newFilters.priceRange[1])
    
    setSearchParams(params)
    
    // Debug log
    console.log('Sidebar filters updated:', newFilters)
    
    // Notify parent component with formatted filters
  ...
  const clearAllFilters = () => {
    const emptyFilters = {
      categories: [],
      brands: [], // Clear brands filter
      priceRange: [priceRange.min, priceRange.max] // Reset to default range
    }
  ...
  const hasActiveFilters = () => {
    return localFilters.categories.length > 0 ||
           localFilters.brands.length > 0 || // Check for active brand filters
            ...
            <h3 className="widget-title">
              ...
              {localFilters.categories.length > 0 && (
                <a
                  ...
                  onClick={(e) => {
                    e.preventDefault()
                    updateFilters({ ...localFilters, categories: [] })
                  }}
                >
                  Clear
                </a>
            ...
            <h3 className="widget-title">
              ...
              {localFilters.brands.length > 0 && (
                <a
                  ...
                  onClick={(e) => {
                    e.preventDefault()
                    updateFilters({ ...localFilters, brands: [] })
                  }}
                >
                  Clear
                </a>
            ...
            <h3 className="widget-title">
              ...
              {(localFilters.priceRange[0] > priceRange.min || localFilters.priceRange[1] < priceRange.max) && (
                <a
                  ...
                  onClick={(e) => {
                    e.preventDefault()
                    updateFilters({ ...localFilters, priceRange: [priceRange.min, priceRange.max] })
                  }}
                >
                  Clear
                </a>
