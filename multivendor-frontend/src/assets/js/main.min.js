    ...
    var e, i, n, o, s, a, r, l, c = {
        ...
        stickySidebar: function () {
                ...
                t(".sidebar-wrapper").each(function () {
                    console.group('🔧 Sidebar Options Check');
                    const $sidebar = t(this);
                    const hasOptions = $sidebar.data("sticky-sidebar-options");
                    ...
                    if (hasOptions) {
                        const options = $sidebar.data("sticky-sidebar-options");
                        console.log('Custom options found:', {
                            rawOptions: options,
                            newOffsetTop: options.offsetTop,
                            newMinWidth: options.minWidth
                        });
                ...
                t(".sidebar-wrapper, .sticky-slider").each(function() {
                    const $el = t(this);
                    $el.data('initial-width', $el.outerWidth());
                    $el.data('initial-height', $el.outerHeight());
                });

                // Handle scroll events for sticky behavior
                t(window).on('scroll', function() {
                    if(t(window).width() >= minWidth) {
                        const currentScrollTop = t(window).scrollTop();
                        const scrollingDown = currentScrollTop > lastScrollTop;
                        
                        t(".sidebar-wrapper, .sticky-slider").each(function() {
                            const $el = t(this);
                            const $container = $el.closest('.row, .container');
                            const containerTop = $container.offset().top;
                            const containerHeight = $container.height();
                            const elementHeight = $el.outerHeight();
                            const elementTop = $el.offset().top;
                            const viewportHeight = t(window).height();
                            const viewportBottom = currentScrollTop + viewportHeight;
                            const elementBottom = elementTop + elementHeight;
                            
                            // Get current state
                            const currentState = $el.data('sticky-state') || 'normal';
                            ...
                            if (scrollingDown) {
                                if (viewportBottom > elementBottom && currentState === 'normal') {
                                    console.log('📌 Fixing to bottom');
                                    $el.data('sticky-state', 'fixed-bottom')
                                    .css({
                                        position: 'fixed',
                                        bottom: '0',
                                        top: 'auto',
                                        width: $el.data('initial-width') + 'px'
                                    });
                                } else if (currentState === 'fixed-top') {
                                // Calculate smooth transition from fixed-top to absolute
                                const currentVisualPosition = elementTop - containerTop;
                                const newTop = currentScrollTop - containerTop + offsetTop;
                                
                                console.log('🔍 Top-to-Absolute Calculation:', {
                                    currentPosition: {
                                        scroll: currentScrollTop,
                                        elementTop,
                                        elementBottom,
                                        containerTop,
                                        visualPosition: currentVisualPosition
                                    },
                                    measurements: {
                                        viewportHeight,
                                        elementHeight,
                                        distanceFromTop: elementTop - currentScrollTop
                                    },
                                    calculation: {
                                        formula: 'currentScrollTop - containerTop + offsetTop',
                                        newTop,
                                        expected: {
                                            elementTop: containerTop + newTop,
                                            elementBottom: containerTop + newTop + elementHeight
                                        }
                                    }
                                });

                                $el.data('sticky-state', 'absolute')
                                .css({
                                    position: 'absolute',
                                    top: newTop + 'px',
                                    width: $el.data('initial-width') + 'px'
                ...
                // Use initial height for calculations
                const initialHeight = $el.data('initial-height');
                const elementViewportBottom = (elementTop + initialHeight) - currentScrollTop;
                const shouldSwitchToBottom = elementViewportBottom <= viewportHeight;
                
                console.log('🔍 Absolute-to-Bottom Check:', {
                    currentPosition: {
                        scroll: currentScrollTop,
                        elementTop,
                        elementBottom: elementTop + initialHeight,
                        elementViewportBottom
                    },
                    measurements: {
                        viewportHeight,
                        initialHeight,
                        currentHeight: elementHeight,
                        distanceToBottom: viewportHeight - elementViewportBottom
                    },
                    shouldSwitch: shouldSwitchToBottom
                });

                if (shouldSwitchToBottom) {
                    console.log('📌 Switching to fixed-bottom');
                    $el.data('sticky-state', 'fixed-bottom')
                       .css({
                           position: 'fixed',
                           bottom: '0',
                           ...
                           height: initialHeight + 'px'
                                ...
                                if (currentState === 'fixed-bottom') {
                                    // Keep element at current visual position
                                    const currentVisualPosition = elementTop - containerTop;
                                    const newTop = currentVisualPosition + 543;
                                    
                                    console.log('🔍 Bottom-to-Absolute Calculation:', {
                                        currentPosition: {
                                            scroll: currentScrollTop,
                                            elementTop,
                                            elementBottom,
                                        ...
                                        measurements: {
                                            viewportHeight,
                                            elementHeight,
                                            distanceFromBottom: viewportHeight - (elementBottom - currentScrollTop)
                                        },
                                        calculation: {
                                            formula: 'elementTop - containerTop',
                                            newTop,
                                            expected: {
                                                elementTop: containerTop + newTop,
                                                elementBottom: containerTop + newTop + elementHeight
                                    ...
                                    // Calculate positions relative to viewport and scroll
                                    const elementViewportTop = elementTop - currentScrollTop;
                                    const elementScrollDelta = lastScrollTop - currentScrollTop;
                                    const proposedTop = offsetTop - 123;
                                    
                                    console.log('🔍 Absolute-to-Fixed Calculation:', {
                                        currentPosition: {
                                            scroll: currentScrollTop,
                                            elementTop,
                                            elementBottom,
                                            containerTop,
                                            viewportTop: elementViewportTop
                                        },
                                        measurements: {
                                            viewportHeight,
                                            elementHeight,
                                            scrollDelta: elementScrollDelta
                                        },
                                        calculation: {
                                            formula: 'elementTop - currentScrollTop <= offsetTop',
                                            proposedTop,
                                            expected: {
                                                elementTop: currentScrollTop + proposedTop,
                                                elementBottom: currentScrollTop + proposedTop + elementHeight
                                            }
                                        }
                                    });

                                    // Only switch to fixed if element is near viewport top
                                    if (elementViewportTop <= offsetTop && elementViewportTop >= -50) {
                                        console.log('📌 Switching to fixed-top');
                                        $el.data('sticky-state', 'fixed-top')
                                        .css({
                                            position: 'fixed',
                                            top: proposedTop + 'px',
                                            width: $el.data('initial-width') + 'px',
                            ...
                            if (currentScrollTop <= containerTop - offsetTop) {
                                console.log('🔝 Reset to top');
                                $el.data('sticky-state', 'normal')
                                .css({
                                    position: 'static',
                                    width: 'auto',
                                    marginTop: 0
                                });
                            } else if (currentScrollTop + viewportHeight >= containerTop + containerHeight) {
                                console.log('🔚 Lock to bottom');
                                $el.data('sticky-state', 'absolute')
                                .css({
                                    position: 'absolute',
                                    top: (containerHeight - elementHeight) + 'px',
                                    width: $el.data('initial-width') + 'px'
                                });
                            }
                        });
                ...
                // Initial position check and store initial widths
                t(".sidebar-wrapper, .sticky-slider").each(function() {
                    const $el = t(this);
                    // Store the initial width as a data attribute
                    $el.data('initial-width', $el.outerWidth());
                    ...
                    console.log('🎯 Initial Position:', {
                        ...
                        position: $el.offset(),
                        dimensions: {
                            width: $el.data('initial-width'),
                            height: $el.outerHeight(),
                        },
                        container: {
                            top: $el.closest('.row, .container').offset().top,
                            height: $el.closest('.row, .container').height()
                        }
                    });
                });
        ...
        footerReveal: function () {
            ...
            if (e.length) {
                var i = function () {
                    t(window).width() >= 992 ? ( e.css("position", "relative")) : ( e.css("position", "relative"))
                };
                t(window).resize(i), i()
            }
        },
