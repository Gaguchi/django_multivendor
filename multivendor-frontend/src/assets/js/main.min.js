                ...
                t(window).on('scroll', function() {
                    if(t(window).width() >= minWidth) {
                        t(".sidebar-wrapper, .sticky-slider").each(function() {
                            const $el = t(this);
                            const containerTop = $el.closest('.row, .container').offset().top;
                            const scrollTop = t(window).scrollTop();
                            const elementTop = $el.offset().top;
                            const elementHeight = $el.outerHeight();
                            const viewportHeight = t(window).height();
                            
                            // Calculate boundaries
                            const upperBound = containerTop - offsetTop;
                            const lowerBound = containerTop + $el.closest('.row, .container').height() - elementHeight;
                            
                            // Determine if element should be sticky
                            let shouldStick = scrollTop >= upperBound && scrollTop < lowerBound;
                            ...
                            if(shouldStick) {
                                if(!$el.hasClass('is-sticky')) {
                                    console.log('📌 Making element sticky:', this);
                                ...
                                if($el.hasClass('is-sticky')) {
                                    console.log('🔓 Removing sticky state:', this);
                                    
                                    // Calculate the absolute position before removing sticky
                                    let absoluteTop;
                                    if (scrollTop < upperBound) {
                                        absoluteTop = 540; // Reset to original position when scrolling up
                                    } else if (scrollTop >= lowerBound) {
                                        absoluteTop = $el.closest('.row, .container').height() - elementHeight + 627;
                                    } else {
                                        absoluteTop = scrollTop + elementTop - containerTop;
                                    }
