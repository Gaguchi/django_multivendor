...
export function ChatWebSocketProvider({ children }) {
  ...
  const connectToRoom = useCallback(async (roomId) => {
    if (!isVendorLoaded || !vendorId || connections.has(roomId)) {
      return;
    }

    console.log(`[Vendor Chat WebSocket] Connecting to room ${roomId}`);
    ...
    try {
      const connection = chatWebSocketService.connectToRoom(roomId, {
        ...
        onDisconnected: () => {
          console.log(`[Vendor Chat WebSocket] Disconnected from room ${roomId}`);
          setActiveRooms(prev => {
      ...
      console.error(`[Vendor Chat WebSocket] Failed to connect to room ${roomId}:`, error);
    }
  ...
  const handleChatMessage = useCallback((roomId, message) => {
    console.log(`[Vendor Chat WebSocket] New message in room ${roomId}:`, message);
    
    // Add notification if message is from customer (not vendor)
  ...
  const handleUserStatus = useCallback((roomId, status) => {
    console.log(`[Vendor Chat WebSocket] User status in room ${roomId}:`, status);
  ...
  const handleTypingStatus = useCallback((roomId, typing) => {
    console.log(`[Vendor Chat WebSocket] Typing status in room ${roomId}:`, typing);
  ...
  const handleMessagesRead = useCallback((roomId, readData) => {
    console.log(`[Vendor Chat WebSocket] Messages read in room ${roomId}:`, readData);
