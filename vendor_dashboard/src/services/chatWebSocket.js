class ChatWebSocketService {
  ...
  connectToRoom(roomId, handlers = {}) {
    if (this.connections.has(roomId)) {
      console.log(`[Vendor Chat WebSocket] Already connected to room ${roomId}`);
    ...
    try {
      ...
      ws.onopen = () => {
        console.log(`[Vendor Chat WebSocket] Connected to room ${roomId}`);
      ...
      ws.onmessage = (event) => {
        try {
          ...
          console.log(`[Vendor Chat WebSocket] Message in room ${roomId}:`, data);
          ...
          switch (data.type) {
            ...
            case 'pong':
              ...
              break;
            default:
              console.log(`[Vendor Chat WebSocket] Unknown message type: ${data.type}`);
          }
        } catch (error) {
          console.error(`[Vendor Chat WebSocket] Error parsing message in room ${roomId}:`, error);
        }
      };

      ws.onclose = (event) => {
        console.log(`[Vendor Chat WebSocket] Disconnected from room ${roomId}`, event);
      ...
      ws.onerror = (error) => {
        console.error(`[Vendor Chat WebSocket] Error in room ${roomId}:`, error);
        onError(error);
      };
      ...
      console.error(`[Vendor Chat WebSocket] Failed to create connection to room ${roomId}:`, error);
      onError(error);
      return null;
    }
  }
  ...
  disconnectFromRoom(roomId) {
    console.log(`[Vendor Chat WebSocket] Manually disconnecting from room ${roomId}`);
  ...
  sendMessage(roomId, content) {
    ...
    if (connection && connection.isConnected) {
      ...
      console.log(`[Vendor Chat WebSocket] Sent message to room ${roomId}:`, content);
    } else {
      console.error(`[Vendor Chat WebSocket] Cannot send message to room ${roomId} - not connected`);
    }
  }
  ...
  attemptReconnect(roomId, handlers) {
    ...
    if (attempts >= this.maxReconnectAttempts) {
      console.log(`[Vendor Chat WebSocket] Max reconnection attempts reached for room ${roomId}`);
      return;
    }

    const delay = this.reconnectDelay * Math.pow(2, attempts); // Exponential backoff
    console.log(`[Vendor Chat WebSocket] Attempting to reconnect to room ${roomId} in ${delay}ms (attempt ${attempts + 1})`);
  ...
  disconnectAll() {
    console.log('[Vendor Chat WebSocket] Disconnecting from all rooms');
